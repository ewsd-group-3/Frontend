...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<'ol'>>(({ className, ...props }, ref) => (
  <ol ref={ref} className={cn('flex flex-wrap items-center gap-1.5 break-words text-sm text-foreground sm:gap-2.5', className)} {...props} />
))
...
>(({ asChild, className, ...props }, ref) => {
  ...
  return <Comp ref={ref} className={cn('transition-colors hover:text-muted-foreground', className)} {...props} />
})
        ...
        {links.map((link, idx) => (
          <>
            <BreadcrumbItem key={link.label}>
              {link.href ? <BreadcrumbLink href={link.href}>{link.label}</BreadcrumbLink> : <p className='font-bold'>{link.label}</p>}
            </BreadcrumbItem>
...
export { Breadcrumb, BreadcrumbList, BreadcrumbItem, BreadcrumbLink, BreadcrumbPage, BreadcrumbSeparator, BreadcrumbEllipsis, CustomBreadcrumb }
